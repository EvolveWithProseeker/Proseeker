function [setstrt,setend]= RCLSTSCPCENT(setstrt,setend)

%Programmed by:     Samuel A Raven
...
finscrmatval=1; %#ok<*NASGU> %   This is set here to keep it out of loops and because it is necessary to the end of the process. It is another thing like gencounter and messing with it will result in mismatched outputs.
...
pmat=table2cell(readtable('1klist.txt','ReadVariableNames',false));
pmat=pmat(setstrt:setend);
%%At this point necessary comparison data is loaded.
for switchindex=1:(setend-setstrt)+1
    switch switchindex
        case 1
            subject=coder.load(pmat{switchindex,1});
        case 2
            subject=coder.load(pmat{switchindex,1});
        case 3
            subject=coder.load(pmat{switchindex,1});
    ...
    switch switchindex
        case 1
            fn=[pmat{switchindex,1} '.gofcheck.csv'];
            dlmwrite(fn, goodnessmatfin);
            goodnessmatfin(:,:)=0;
            subtractmat(:,:)=0;
        case 2
            fn=[pmat{switchindex,1} '.gofcheck.csv'];
